...
from sighting_identification.db.postgres import get_species_embeddings, update_sighting_as_unrecognized, \
    ...
    update_sighting_as_unknown_species
...
def start(sighting_id, image_path):
    try:
        ...
        species, confidence = get_species(cropped_image, category)
        if confidence < 0.6:
            update_sighting_as_unknown_species(sighting_id)
            return
        ...
        if individual_similarities and individual_similarities[0]['similarity'] > 0.7:
